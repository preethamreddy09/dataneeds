...
function buildStats(datasets) {
  document.getElementById('stat-sources').textContent = datasets.length;
}
    ...
    if (!ds.sample_file || ds.rows.length === 0) {
      ...
      content.innerHTML = `
        <div class="placeholder-msg">
          ...
          <a href="#contact" class="btn-primary">Request full sample</a>
        </div>
      `;
    } else {
      // Build table
      ...
      content.innerHTML = `
        ...
        <p class="table-note">Showing ${ds.rows.length} of ${ds.record_count} records. <a href="#contact">Contact us</a> for a full sample file.</p>
      `;
    }
